...
`electron/updater.ts` wraps `autoUpdater` with `autoDownload = false` so the UI can prompt at each step. It emits a single `updater:status` object to the renderer (`{ state: 'checking'|'available'|'downloading'|'downloaded'|'not-available'|'error', currentVersion, manual, version?, percent?, bytesPerSecond?, … }`) and caches `lastStatus` (renderer pulls it via `updater:get-status` on mount to avoid missed-event races, same pattern as the monitor). `main.ts` calls `initUpdater(() => mainWindow)` + `checkForUpdates(false)` on startup (guarded by `!isDev`); the Help-menu "Check for Updates…" and the in-app button call `checkForUpdates(true)`. The `manual` flag gates the quiet cases: a **non-manual** `not-available`/`error` renders nothing (silent startup, e.g. offline), while a manual one shows "up to date" / the error.
...
### Re-price assistant (Trader Dashboard "Re-price" tab)

**Hard constraint — why this is assist-only, not automation:** ESI has **no write endpoint** to modify/place/cancel market orders (read-only), and driving the EVE client's inputs via macros/injection is a EULA-bannable offence. So the assistant does everything except the final in-client keypress: on each undercut it computes the tick-correct `suggestedPrice` (already produced by the monitor), copies it to the clipboard (`copyPrice` → main-process `clipboard.writeText`, which works while the app is unfocused), and opens the item's market window via `openMarketWindow`. The user does double-click order → Modify → Ctrl+V → Enter.

`sortUndercutOrders(orders, config)` (`src/lib/orderHealth.ts`) produces the one sorted undercut list that feeds the tab list, the guided stepper, and the global hotkey (so ordering is identical everywhere), keyed off `config.repriceSort` (`iskAtRisk` | `margin` | `none`; margin uses `scoreBuyOrder().competeMargin`, buys first). `repriceCore()` in `TraderPanel.tsx` is the **single** point the stepper index advances (once per successful re-price, whatever the trigger).

**Snooze:** acting on an order (Re-price button/hotkey, or the Cancel card's cancel button, on `res.ok` only) hides it from **both** action lists for `SNOOZE_MS` (5 min — aligned to the monitor poll + ESI order-cache TTL, so a fresh scan lands by expiry). Renderer-only (`snoozed: Map<orderId, {expiry, priceAtAction}>` in `TraderPanel.tsx`), keyed by `orderId` (stable across an in-client Modify). **Early-clear** drops an entry before expiry when the order leaves `activeOrders` (cancelled/filled) or its `price` changes (Modify landed) — so the live derived state decides, and a re-undercut on your *new* price reappears in minutes. A "N snoozed" chip in the Re-price/Cancel tab headers expands an Undo list. On expiry/clear the item reappears only if the fresh scan still shows it undercut / still a weak candidate.

**Global hotkey** (`config.repriceHotkey`, default `CommandOrControl+Shift+R`, `repriceHotkeyEnabled`): registered in `electron/main.ts` via `globalShortcut` only while the monitor runs (register on `monitor:start`, unregister on stop/logout/`will-quit`); `monitor:start` returns `{ hotkeyRegistered }` so the renderer can surface a collision. It sends `hotkey:reprice-next` to the renderer (main stays dumb — renderer owns the list + index). Feedback while unfocused is a native `Notification` from main (`notify:show` IPC) with a running "X of N" counter; `silent: !repriceSound` so the OS notification sound *is* the audio cue (no `shell.beep` — that isn't a real Electron API). A window `blur`/`focus` listener pre-arms the *next* stepper card visually after a button re-price steals focus to EVE — it toggles display only, never advances the index.
